...
F=$(git grep -nE 'PHPSESSID=[A-Za-z0-9]{8,}|OpenEMR=[A-Za-z0-9]{8,}|Set-Cookie:|Cookie:[[:space:]]*[A-Za-z0-9_]+=|Bearer [A-Za-z0-9._-]{16,}|sk-ant-[A-Za-z0-9]|ANTHROPIC_API_KEY=[^[:space:]$]+|apify_api_[A-Za-z0-9]{6,}|SERVICE_ROLE_KEY[[:space:]]*[:=][[:space:]]*["'"'"']?[A-Za-z0-9._-]{8,}|APIFY_TOKEN[[:space:]]*[:=][[:space:]]*["'"'"']?[A-Za-z0-9._-]{8,}|eyJ[A-Za-z0-9_-]{10,}\.eyJ[A-Za-z0-9_-]{10,}|csrf_token[^a-z]{0,6}[A-Za-z0-9]{16,}' -- ':!scripts/secrets-scan.sh' ':!automation-server/skill/scan-staging.sh' ':!sync.sh' | grep -vE 'Bearer YOUR_[A-Z_]+')
...
F=$(git grep -n 'token_main=' -- ':!scripts/secrets-scan.sh' ':!automation-server/skill/scan-staging.sh' ':!sync.sh')
if [ -n "$F" ]; then echo "$F" | cut -c1-200; HARD=1; else echo "  none"; fi
...
O=$(git grep -hoE -- '--pass [A-Za-z0-9_$]+' -- ':!scripts/secrets-scan.sh' ':!automation-server/skill/scan-staging.sh' ':!README.md' | sort -u | grep -vE -- '^--pass (pass|receptionist)$')
