...
const server = http.createServer(async (req, res) => {
  ...
  try {
    // Landing page (handoff 19): GET / serves public/index.html; GET /health keeps the JSON.
    if (p === '/' && req.method === 'GET') return serveFile(res, path.join(HERE, 'public', 'index.html'));
    if (p === '/workflow/from-recording' && req.method === 'POST') { // Chrome Recorder JSON or recorder events.json → bullets
      let raw = ''; for await (const c of req) raw += c;
      const dirU = path.join(HERE, 'uploads'); fs.mkdirSync(dirU, { recursive: true });
      const f = path.join(dirU, `${stamp()}-recording.json`); fs.writeFileSync(f, raw);
      const r = await run(path.join(ROOT, 'from-recording', 'from-recording'), [f], HERE);
      try { const out = JSON.parse(r.out); return send(res, 200, out); } catch { return send(res, 422, { error: (r.err || r.out).trim().split('\n').pop() || 'from-recording failed', exit_code: r.code }); }
    }
    if (p === '/skill' && req.method === 'GET') { // streams the zip built by skill/build-skill.sh (owned by another agent)
      const script = path.join(HERE, 'skill', 'build-skill.sh');
      if (!fs.existsSync(script) || !fs.existsSync(path.join(HERE, 'skill', 'READY'))) return send(res, 503, { error: 'skill export not ready' });
      const r = await run('/bin/bash', [script], HERE);
      const zip = (r.out.trim().split('\n').pop() || '').trim();
      if (r.code !== 0 || !zip || !fs.existsSync(zip)) return send(res, 503, { error: 'skill export not ready', exit_code: r.code });
      res.writeHead(200, { 'content-type': 'application/zip', 'content-disposition': 'attachment; filename=referral-intake-skill.zip', 'access-control-allow-origin': '*' });
      return fs.createReadStream(zip).pipe(res);
    }
    if (p === '/health') return send(res, 200, { ok: true, site: SITE, default_site: DEFAULT_SITE, sites: SITES, busy, endpoints: ['POST /login', 'GET /status', 'POST /record', 'GET /recordings/latest', 'POST /run', 'GET /report/latest', 'GET /report/latest.json', 'POST /intake', 'GET /intake/latest', 'GET /review/latest', 'POST /review/<id>/mark', 'GET / (landing page)', 'POST /workflow/from-recording', 'GET /skill'] });
